...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN utm_campaign = 'nonbrand' THEN orders.order_id ELSE NULL END) AS nobrand_orders,
...
SELECT DISTINCT 
    ...
    http_referer
FROM website_sessions
WHERE website_sessions.created_at < '2012-11-17';
...
CREATE TEMPORARY TABLE nobrand_test_sessions_w_orders
SELECT
...
SELECT * FROM nobrand_test_sessions_w_orders;


-- to find the difference between conversion races
...
FROM nobrand_test_sessions_w_orders
GROUP BY 1;
...
-- X .0087 increment conversion = 202 incremental orders since 7/29
